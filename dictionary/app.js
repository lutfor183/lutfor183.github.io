...
async function search(v){
  ...
  try{
    ...
    resultsEl.innerHTML=rows.map(x=>`<div class="bg-white border rounded-2xl p-5" style="box-shadow:0 0 40px rgba(124,58,237,0.15)"><div class="font-medium">${x.word}</div><div class="text-xs text-zinc-500 mt-1">${x.pos||""}</div><div class="text-sm leading-6 mt-3">${(x.meaning_text||x.definition||"")}</div>${x.meaning_html?`<div class="text-sm leading-6 mt-3">${x.meaning_html}</div>`:""}</div>`).join("");
  }catch{
    statusEl.textContent="";
    resultsEl.innerHTML=`<div class="bg-white border rounded-2xl p-5 text-sm">Error - check Worker</div>`;
  }
}
